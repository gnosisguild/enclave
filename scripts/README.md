...
```bash
# Build all circuits (defaults: --preset insecure-512 --committee micro)
pnpm build:circuits
...
pnpm build:circuits --committee medium
...
Four sizes are supported, mirroring `e3_zk_helpers::CiphernodesCommitteeSize`:
...
different `.sol` bytes. The committed files only match `(insecure-512, micro)`.
...
For non-canonical committees (e.g. `medium`), pass `--committee <name>`; the verifiers land under
...
# Generate for a non-canonical committee (writes under honk/medium/)
pnpm generate:verifiers --committee medium --write
