...
(pnpm concurrently \
  ...
  --kill-others \
  --success first \
  "wait-on file:/tmp/enclave_ciphernodes_ready tcp:localhost:8545 http://localhost:13151/health && export \$(enclave print-env --chain localhost) && pnpm vitest run ./tests/integration.spec.ts" \
