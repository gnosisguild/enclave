    ...
    return (
        <div className="bg-red-50 border border-red-200 rounded-lg p-4">
            ...
            {showDetails && userMessage !== technicalMessage && (
                <pre className="mt-2 p-2 bg-red-100 border border-red-300 rounded text-xs text-red-800 overflow-x-auto">
                    {technicalMessage}
                </pre>
            )}
        </div>
        ...
        <div className='space-y-6 text-center'>
            ...
            <div className="space-y-4">
                ...
                <p className="text-slate-600 leading-relaxed">
                    Enclave is a protocol for Encrypted Execution Environments (E3) that enables secure computations
                    on private data using fully homomorphic encryption (FHE), zero-knowledge proofs, and distributed
                    key cryptography. Connect your wallet to experience privacy-preserving computation.
                </p>
                <div className="bg-enclave-50 border border-enclave-200 rounded-lg p-4">
                    <p className="text-sm text-slate-600">
                        <strong>How it works:</strong> You'll request an E3 computation → Ciphernode committee is selected
                        → Committee publishes shared public key → You encrypt and submit inputs → Secure computation executes
                        → Only verified outputs are decrypted by the committee.
                    </p>
                </div>
            </div>
    ...
    <CardContent>
        <div className='space-y-6 text-center'>
            ...
            <div className="space-y-4">
                ...
                <p className="text-slate-600 leading-relaxed">
                    Request an E3 computation from Enclave's decentralized network. This initiates the selection
                    of a Ciphernode Committee through cryptographic sortition, who will generate shared
                    keys for securing your computation without any single point of trust.
                </p>
    ...
    <CardContent>
        <div className='space-y-6 text-center'>
            ...
            <div className="space-y-4">
                ...
                <p className="text-slate-600 leading-relaxed">
                    Activate the E3 using the Ciphernode Committee's shared public key. This distributed
                    key ensures no single node can decrypt your inputs or intermediate states - only the verified
                    final output can be collectively decrypted by the committee.
                </p>
    ...
    <CardContent>
        <form onSubmit={onSubmit} className='space-y-6 text-center'>
            ...
            <div className="space-y-4">
                ...
                <p className="text-slate-600 leading-relaxed">
                    Enter two numbers for a privacy-preserving addition using fully homomorphic encryption (FHE).
                    Your inputs will be encrypted locally and remain encrypted throughout the entire computation
                    process, with only the final result being decrypted.
                </p>
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    <p className="text-sm text-slate-600">
                        <strong>Privacy Guarantee:</strong> FHE allows computation on encrypted data. Your numbers remain
                ...
                <div className="space-y-4">
                    <div>
                        <label htmlFor="input1" className="block text-sm font-medium text-slate-700 mb-2">
                            First Number
                        </label>
                        <input
                            id="input1"
                            ...
                            className="w-full px-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-enclave-500 focus:border-transparent"
                            placeholder="Enter first number"
                            required
                        />
                    </div>
                    <div>
                        <label htmlFor="input2" className="block text-sm font-medium text-slate-700 mb-2">
                            Second Number
                        </label>
                        <input
                            id="input2"
                ...
                {!inputPublishError && !inputPublishSuccess && (
                    <div className="space-y-4">
                        ...
                        <p className="text-slate-600">
                            Your inputs are being encrypted to the committee's public key and submitted to the E3.
                            The Compute Provider will execute the FHE computation over your encrypted data...
                        </p>
                        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                            <p className="text-sm text-slate-600">
                ...
                {inputPublishError && (
                    <div className="space-y-4">
                        <ErrorDisplay
                            ...
                            onToggleDetails={onToggleErrorDetails}
                        />
                        <button
                            onClick={onTryAgain}
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                            <p className="text-sm text-green-600">
                                ...
                                <br />
                                Your encrypted inputs have been published to the E3. The Compute Provider is executing
                                the FHE computation and will publish the ciphertext output for committee decryption.
                            </p>
                        </div>
                        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
    ...
    <CardContent>
        <div className='space-y-6 text-center'>
            ...
            <div className="space-y-4">
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    <p className="text-sm text-slate-600">
                        <strong>🔒 Cryptographic Guarantees:</strong> Your inputs remained encrypted throughout the entire
                        process. The Ciphernode Committee used distributed key cryptography to decrypt only the
                        verified output, ensuring data privacy, data integrity, and correct execution.
                    </p>
                </div>
            </div>
...
const WizardSDK: React.FC = () => {
    ...
    const handleInputSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await publishInput(
                e3State.id,
                `0x${Array.from(encryptedInput1, b => b.toString(16).padStart(2, '0')).join('')}` as `0x${string}`
            )
            ...
            <div className="flex items-center space-x-2">
                {[1, 2, 3, 4, 5, 6].map((step) => (
                    <div key={step} className="flex items-center">
                        <div className={`flex items-center justify-center w-10 h-10 rounded-full border-2 transition-all duration-200 ${currentStep >= step
                            ? 'bg-enclave-100 border-enclave-400 text-enclave-600'
                            : 'bg-slate-100 border-slate-300 text-slate-400'
                            }`}>
                            {getStepIcon(step as WizardStep)}
                        </div>
                        {step < 6 && (
                            <div className={`w-8 h-0.5 mx-2 transition-all duration-200 ${currentStep > step ? 'bg-enclave-400' : 'bg-slate-300'
                                }`} />
                        )}
                    </div>
                ))}
