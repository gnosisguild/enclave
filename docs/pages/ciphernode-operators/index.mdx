...
Circuit verifiers (C5, C7 BN/Mod, Fold) are deployed with the main flow. See `deployed_contracts.json` for addresses.

| Verifier                                                 | Address                                      |
| -------------------------------------------------------- | -------------------------------------------- |
| ThresholdPkAggregationVerifier (C5)                      | See `deployed_contracts.json` after deploy   |
| ThresholdDecryptedSharesAggregationBnVerifier (C7 BN)    | See `deployed_contracts.json` after deploy   |
| ThresholdDecryptedSharesAggregationModVerifier (C7 Mod)  | See `deployed_contracts.json` after deploy   |
