...
struct SlashPolicy {
  ...
  uint256 licensePenalty; // INTF to confiscate
...
3. **File appeal if applicable**:
   ...
   ```
   Only the accused operator can appeal, and only within the appeal window. Proof-verified slashes
   cannot be appealed.
...
| Penalty Type    | Effect                                           |
| --------------- | ------------------------------------------------ |
| Ticket penalty  | Burns ITK from your balance                      |
| License penalty | Confiscates INTF from your bond                  |
...
| License | INTF enters exit queue                |
...
interfold ciphernode license claim --max-ticket 100 --max-license 50
```
...
| Command                              | Description                        |
| ------------------------------------ | ---------------------------------- |
| `interfold ciphernode status`        | Check registration and exit status |
| `interfold ciphernode deregister`    | Start exit process                 |
| `interfold ciphernode license claim` | Claim unlocked exits               |
| `interfold ciphernode register`      | Cancel exit and re-register        |
| `interfold ciphernode deactivate`    | Withdraw stakes to become inactive |
