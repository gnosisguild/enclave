...
published, the CiphernodeRegistry calls `onCommitteePublished()` on the Enclave contract, which
...
Enclave uses a Lean Merkle Tree implementation. Each input is hashed using the PoseidonT3 Hash
...
4. The plaintext result is published and the Decryption Verifier validates the decryption proof.
