---
title: 'Installation'
description: 'Install and set up the Interfold CLI'
---

# Installation

This guide covers how to install the Interfold CLI and get your development environment set up.

## Prerequisites

Before installing Interfold, make sure you have the following dependency installed:

- [Rust](https://www.rust-lang.org/tools/install)

## Quick Install (Recommended)

The easiest way to install the Interfold CLI is using our installer script:

```bash
curl -fsSL https://raw.githubusercontent.com/gnosisguild/interfold/main/install | bash
```

Or if you prefer `wget`:

```bash
wget -qO- https://raw.githubusercontent.com/gnosisguild/interfold/main/install | bash
```

This script will download and install `interfoldup`, which is the standalone installer for the
Interfold CLI.

## Using interfoldup

Once you have `interfoldup` installed, you can manage your Interfold CLI installation:

### Install the Interfold CLI

```bash
# Install to ~/.local/bin (default)
interfoldup install

# Install to /usr/local/bin (requires sudo)
interfoldup install --system
```

Running `interfoldup install` will install the latest version of the Interfold CLI. See
`interfoldup --help` for more options.

### Update the Interfold CLI

```bash
# Update to the latest version
interfoldup update

# Update system-wide installation
interfoldup update --system
```

### Uninstall

```bash
# Remove the Interfold CLI
interfoldup uninstall

# Remove system-wide installation
interfoldup uninstall --system
```

## Install with Nix

If you use [Nix](https://nixos.org/) with flakes enabled, you can get a complete runtime environment
with no additional dependencies — Nix handles everything including installing the `bb` backend.

Get a devshell directly without installing it:

```bash
nix develop github:gnosisguild/interfold/{branch,tag,commit}
```

This will build from source using the nix package manager and provide a runtime environemnt for
interfold. It might take some time but the result will be cached on your local nix store for future
execution.

```bash
❯ nix develop github:gnosisguild/interfold/ry/v0.1.16
❯ interfold --version
interfold 0.1.16
❯ bb --version
3.0.0-nightly.20251104
❯ echo $E3_CUSTOM_BB
/nix/store/6da5va1malbj4pjrzn5lan6n51m4hg9h-bb/bin/bb
```

NOTE: On first run, Nix will ask you to trust our binary cache. Say yes to both prompts to download
pre-built binaries instead of building from source:

```
do you want to allow configuration setting 'extra-substituters' to be set to '...'? y
do you want to permanently mark this value as trusted? y
```

If you decline, everything still works — it just builds from source, which takes longer.

## Manual Installation

If you prefer to install manually:

1. Visit the [releases page](https://github.com/gnosisguild/interfold/releases)
2. Download the appropriate release archive for your platform:
   - Linux: `interfold-linux-x86_64.tar.gz`
   - macOS: `interfold-macos-aarch64.tar.gz`
3. Extract the binary and place it in your PATH (e.g., `~/.local/bin` or `/usr/local/bin`)
4. Make sure the binary is executable: `chmod +x interfold`

## Platform Support

| Platform | Architecture             | Status           |
| -------- | ------------------------ | ---------------- |
| Linux    | x86_64                   | ✅ Native binary |
| macOS    | Apple Silicon (M1/M2/M3) | ✅ Native binary |
| macOS    | Intel                    | ✅ Via Rosetta 2 |

_Note: Intel Macs automatically run Apple Silicon binaries through Rosetta 2 translation._

### Building from Source

If you need to build `interfold` from source:

1. Ensure you have Rust installed (using [rustup](https://rustup.rs/))
2. Clone the repository

```bash
git clone https://github.com/gnosisguild/interfold.git
cd interfold
```

3. Build the binary

```bash
cargo install --locked --path ./crates/cli --bin interfold -f
```

## Verify Installation

After installation, verify that the Interfold CLI is working correctly:

```bash
interfold --help
```

You should see the help information for the Interfold CLI.

## Next Steps

Once you have the Interfold CLI installed, you're ready to:

1. [Run your first E3 program](/quick-start)
2. [Learn about E3 concepts](/what-is-e3)
3. [Explore the architecture](/architecture-overview)
