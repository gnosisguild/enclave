...
Enclave ships a dedicated Noir workspace under `circuits/` plus reusable libraries (SAFE sponge,
polynomial utilities, GRECO hashing, etc.). Use these circuits as-is or vendor them into your own E3
programs.
...
From `v0.1.6` onward the directory becomes `circuits/crates/libs/safe` (the `packages/` prefix was removed).
...
These scripts exit early (without failing the whole build) if `nargo` is not installed, which makes CI happy
when Noir is optional.
...
When adding new circuits, remember to update `package.json` scripts (e.g., `pnpm dev:all`) so they call your
compile script before spinning up the dev environment. This keeps your verifiers in sync with the Noir source.
