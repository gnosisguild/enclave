...
page (**Phases & Circuits** section). Large statements (especially **C2**) use **inner** proofs
that are then combined inside `recursive_aggregation/` (AB combiners → per-step folds →
`node_fold` per party → `nodes_fold` across parties → `dkg_aggregator` /
