...
[`agent/flow-trace/04_DKG_AND_COMPUTATION.md`](https://github.com/gnosisguild/interfold/blob/main/agent/flow-trace/04_DKG_AND_COMPUTATION.md).

The canonical **package index** (paths, **CircuitName**, C0–C7 IDs) is
[`circuits/README.md`](https://github.com/gnosisguild/interfold/blob/main/circuits/README.md) in the
...
```toml
[dependencies]
interfold_lib = { git = "https://github.com/gnosisguild/interfold", tag = "v0.1.15", directory = "circuits/lib" }
```
...
```bash
pnpm build:circuits                              # default: --preset insecure-512 --committee micro
pnpm build:circuits --preset secure-8192 --committee large
...
**Preset** (`insecure-512` | `secure-8192`) selects the BFV parameter set; **committee** (`micro` |
`small` | `medium` | `large`) selects `(N, T, H)` for secret sharing. Always switch committee via
`pnpm build:circuits --committee <name>` — see
[`scripts/README.md`](https://github.com/gnosisguild/interfold/blob/main/scripts/README.md#circuit-builder)
and
[`circuits/benchmarks/README.md`](https://github.com/gnosisguild/interfold/blob/main/circuits/benchmarks/README.md).

Generate per-circuit witness/config artifacts with `zk_cli` (must pass matching `--committee`):

```bash
cargo run -p e3-zk-helpers --bin zk_cli -- --circuit pk-generation --preset insecure --committee large --toml
```
