...
- Manages operator license bonds (INTF tokens) and ticket balances (ITK)
...
Every E3 transitions through a well-defined state machine managed by the Interfold contract:
...
decryption window) on the Interfold contract. The committee formation window is managed separately
by the CiphernodeRegistry. If any deadline passes without the expected action, the E3 can be marked
as failed and the refund manager distributes funds to honest participants.
