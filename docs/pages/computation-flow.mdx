...
The computation process within the Interfold protocol is decentralized and open, allowing any participant to
...
Providers, or other network participants.

### Phase 1: Request

1. **Define the E3 Program**: Author your Secure Process using the tooling described in
   [Writing the Secure Process](/write-secure-program). The Interfold currently ships first-party support
...
3. **Assemble `E3RequestParams`**:
   - `threshold`: the CiCo size/threshold tuple (t/n) used for decryption security
   - `startWindow` and `duration`: governs when inputs open/close and how long execution may run
...
3. **Submit Inputs**: Both encrypted data and ZKPs are submitted to the Interfold contract, which will
