...
```bash
pnpm add @interfold/sdk @interfold/react
```
...
```typescript
import { InterfoldSDK, InterfoldEventType } from '@interfold/sdk'
...
For React applications, use the `useInterfoldSDK` hook for seamless integration:
...
function E3Dashboard() {
  ...
  }, [isInitialized, onInterfoldEvent, off, InterfoldEventType, RegistryEventType]);
  ...
  if (!isInitialized) {
    return <div>Connecting to Interfold...</div>;
  }
...
sdk.onInterfoldEvent(InterfoldEventType.CIPHERTEXT_OUTPUT_PUBLISHED, (event) => {
...
sdk.onInterfoldEvent(RegistryEventType.COMMITTEE_PUBLISHED, (event) => {
  console.log('Committee selection complete:', event.data)
})

sdk.onInterfoldEvent(RegistryEventType.COMMITTEE_FINALIZED, (event) => {
...
class E3CoordinationServer {
  private sdk: InterfoldSDK

  constructor(privateKey: string, rpcUrl: string) {
    // Use the static factory for convenience
    this.sdk = InterfoldSDK.create({
      rpcUrl,
  ...
  private setupEventListeners() {
    ...
    this.sdk.onInterfoldEvent(InterfoldEventType.E3_REQUESTED, async (event) => {
      const { e3Id } = event.data
      console.log(`New E3 requested: ${e3Id}`)
    })

    // Monitor ciphertext output publication
    this.sdk.onInterfoldEvent(InterfoldEventType.CIPHERTEXT_OUTPUT_PUBLISHED, async (event) => {
      console.log('Ciphertext output published:', event.data)
    })

    // Monitor plaintext output publication
    this.sdk.onInterfoldEvent(InterfoldEventType.PLAINTEXT_OUTPUT_PUBLISHED, async (event) => {
...
const devSdk = InterfoldSDK.create({
...
const prodSdk = InterfoldSDK.create({
  rpcUrl: process.env.RPC_URL!,
  privateKey: process.env.PRIVATE_KEY! as `0x${string}`,
  chain: mainnet,
  contracts: {
    interfold: process.env.INTERFOLD_CONTRACT_ADDRESS! as `0x${string}`,
