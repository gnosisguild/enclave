...
Every E3 program implements the `IE3Program` interface from `@interfold/contracts`. It has three
functions that the Interfold contract calls at different stages of the E3 lifecycle:
...
When a new E3 is requested, the Interfold contract calls `validate` on your program. This is where
you parse parameters, set up state, and decide whether the request is valid.
...
- Only the Interfold contract (or the program owner) can call `validate`
- `customParams` carries application-specific configuration — CRISP uses it for voting options and
  credit mode
- The function returns an encryption scheme identifier so the Interfold contract knows which scheme
...
Each time a user submits encrypted data to the E3, the Interfold contract calls `publishInput`. This
...
development, set `RISC0_DEV_MODE=1` to skip real proof generation, and ensure that the contracts
were deployed using full mock mode, otherwise a real proof will be needed.

---

## Step 4: verify — Confirm the Result

After the compute provider publishes its result, the Interfold contract calls `verify` on your
...
sequenceDiagram
    ...
    participant E as Interfold contract
...
1. **Implement `IE3Program`** — start from the
   [MockE3Program](https://github.com/gnosisguild/interfold/blob/main/packages/interfold-contracts/contracts/test/MockE3Program.sol)
