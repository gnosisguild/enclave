...
the voter is eligible (Merkle membership), and — for mask votes — that the ciphertext is an
encryption of zero correctly added to the previous slot ciphertext. All without revealing the vote
itself.
...
verifier contract (`CRISPVerifier.sol`). This verifier is generated by Barretenberg during circuit
compilation and contains the verification key baked in.
...
— standard Solidity checks in `publishInput` may be sufficient.
