...
clear guardrails for confidential compute. This guide distills the equivalent playbook for the Interfold.

## Parameter Strategy

- **Thresholds**: Choose `threshold = [t, n]` so that `t` withstands your adversary model while `n`
  matches the available Ciphernodes. Higher thresholds improve collusion resistance but increase fees.
