...
- **Push logic to validators**: Implement `IE3Program.validateInput` so ZK proofs and policy checks
...
- **Failover**: Decide how your app reacts if `CiphertextOutputPublished` never arrives—e.g., allow
  a guardian actor to cancel the round and refund deposits. See the
  [Requestor Guide](/requestor-guide) for the full failure → refund map and the `markE3Failed` /
  `claimRequesterRefund` flow.

## Key Material & Ciphernodes

- **Key hygiene**: `dev:setup` wipes `.interfold` directories for you; mimic that behavior in
...
2. Execute integration tests that cover `request → publishInput → publishCiphertextOutput` with mock
   proofs.
3. Verify wallet funding for any automation (interfold CLI, Hardhat deployers).
