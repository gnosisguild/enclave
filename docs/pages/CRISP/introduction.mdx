...
This project serves as a comprehensive example of an E3 Program, demonstrating secure and impartial decision-making processes with a modern Hardhat-based architecture. It showcases a complete full-stack implementation including frontend, backend, smart contracts, Noir circuits, and zkVM components. The code lives in `examples/CRISP` inside this repository and reuses the same [Project Template](/project-template) conventions that `enclave init` produces.
...
```
CRISP/
├── client/                  # React frontend application (Vite + @enclave-e3/sdk)
...
│   └── sdk/                 # CRISP-specific TypeScript helpers
...
The client is a React application built with TypeScript that provides a voting interface and reuses the shared [Enclave SDK](/sdk):
...
The server is a Rust-based coordination service that manages the E3 lifecycle and drives the same SDK from a privileged wallet:
...
- `scripts/compile_circuits.sh`: compiles Noir circuits, writes verification keys, and emits the `CRISPVerifier.sol` contract used by `packages/crisp-contracts`
