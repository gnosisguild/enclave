...
Keep this terminal open; logs from every service are multiplexed with
`pnpm concurrently`.

### (Optional) Build Once for Production

If you want release binaries or a static client before running, execute:

```sh
pnpm dev:build
```

That pipeline runs `cargo build --locked`, compiles the contracts, and builds the client bundle.
...
1. Select `CRISP: Voting Protocol (ETH)`
2. Choose `Initialize new E3 round`
...
1. Import the Hardhat/Anvil deployer key so you have funds available:
   ```
