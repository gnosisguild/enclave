...
if [ "$1" == "--ui" ]; then
  ...
  if [[ "$OSTYPE" == "linux-gnu"* ]]; then
    PLAYWRIGHT_CMD="STABLE_MODE=true pnpm synpress --headless && xvfb-run pnpm playwright test"
  else
    PLAYWRIGHT_CMD="STABLE_MODE=true pnpm synpress --headless && pnpm playwright test"
  fi
fi
