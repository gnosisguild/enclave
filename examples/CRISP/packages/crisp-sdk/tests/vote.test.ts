...
describe('Vote', () => {
  ...
  describe('decodeTally', () => {
    it.only('Should decode an encoded tally into its decimal representation', () => {
      // Output of a real FHE computation.
      const tally =
      ...
      expect(decoded.yes).toBe(10000000000)
      expect(decoded.no).toBe(30000000000)
    })
  })
  ...
  describe('encodeVote', () => {
    ...
    it('Should encode yes vote correctly in the first half', () => {
      const encoded = encodeVote({ yes: 10, no: 2 })
