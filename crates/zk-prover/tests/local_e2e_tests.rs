...
async fn setup_share_computation_sk_base_chunk_test() -> Option<(
    ...
    // Rollback layout: C2a is implemented directly as `sk_share_computation`.
...
async fn setup_share_computation_e_sm_base_chunk_test() -> Option<(
    ...
    // Rollback layout: C2b is implemented directly as `e_sm_share_computation`.
...
async fn test_share_computation_sk_commitment_consistency() {
    ...
    // Prove directly with the new rollback layout (C2a is `sk_share_computation`).
    let proof = ShareComputationCircuit
        .prove(&prover, &preset, &sample, e3_id)
        .expect("share computation (C2a) proof should succeed");

    // The circuit's first public input is `expected_secret_commitment`.
    let computation_output = ShareComputationCircuit::compute(preset, &sample).unwrap();
    let expected = computation_output.inputs.expected_secret_commitment;
    let actual = extract_field(&proof.public_signals, 0);

    assert_eq!(actual, expected, "C2a expected_secret_commitment mismatch");
...
async fn test_share_computation_e_sm_commitment_consistency() {
    ...
    // Prove directly with the new rollback layout (C2b is `e_sm_share_computation`).
    let proof = ShareComputationCircuit
        .prove(&prover, &preset, &sample, e3_id)
        .expect("share computation (C2b) proof should succeed");
    ...
    assert_eq!(actual, expected, "C2b expected_secret_commitment mismatch");
