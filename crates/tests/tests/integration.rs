...
//
// These tests verify the C1→C5 proof connection end-to-end through real actors:
//   1. Generate real BFV threshold keyshares
//   2. Compute correct pk_commitments from keyshare data
//   3. Build signed C1 proofs with correct or tampered commitments
//   4. Dispatch PkAggregationProofPending through the event bus
//   5. Verify the Multithread prover detects mismatches
//   6. Verify ProofRequestActor emits SignedProofFailed for faulting parties
//   7. Verify E3Failed is NOT emitted on mismatch (aggregator retries)

/// Generate BFV threshold keyshares and compute the correct pk_commitment
/// for each party, matching what the C5 prover expects.
fn generate_keyshares_and_commitments(
    num_parties: usize,
) -> Result<(
    Vec<ArcBytes>, // keyshare_bytes (serialized PublicKeyShare)
    ArcBytes,      // aggregated_pk_bytes
    Vec<[u8; 32]>, // correct pk_commitments per party
    Arc<fhe::bfv::BfvParameters>,
)> {
    let preset = BfvPreset::InsecureThreshold512;
    let (threshold_params, _) =
        build_pair_for_preset(preset).map_err(|e| anyhow::anyhow!("{e}"))?;
    let crp = create_deterministic_crp_from_default_seed(&threshold_params);

    let mut keyshare_bytes = Vec::with_capacity(num_parties);
    let mut pk_shares = Vec::with_capacity(num_parties);

    for _ in 0..num_parties {
        let sk = SecretKey::random(&threshold_params, &mut OsRng);
        let pk_share = PublicKeyShare::new(&sk, crp.clone(), &mut OsRng)?;
        keyshare_bytes.push(ArcBytes::from_bytes(&pk_share.to_bytes()));
        pk_shares.push(pk_share);
    }

    let public_key: PublicKey = pk_shares.iter().cloned().aggregate()?;
    let aggregated_pk_bytes = ArcBytes::from_bytes(&public_key.to_bytes());

    // Compute correct pk_commitments using the same logic as PkAggInputs::compute
    let bit_pk = compute_modulus_bit(&threshold_params);
    let mut commitments = Vec::with_capacity(num_parties);

    for pk_share in &pk_shares {
        let mut pk0 = CrtPolynomial::from_fhe_polynomial(&pk_share.p0_share());
        let mut pk1 = CrtPolynomial::from_fhe_polynomial(&crp.poly());
        pk0.reverse();
        pk0.center(threshold_params.moduli())
            .map_err(|e| anyhow::anyhow!("{e}"))?;
        pk1.reverse();
        pk1.center(threshold_params.moduli())
            .map_err(|e| anyhow::anyhow!("{e}"))?;
...
/// Build a signed C1 proof with a specific pk_commitment in public_signals.
/// C1 (PkGeneration) has 3 output fields of 32 bytes each:
///   [sk_commitment, pk_commitment, e_sm_commitment]
fn build_signed_c1_proof(
    e3_id: &E3id,
    pk_commitment: &[u8; 32],
    signer: &PrivateKeySigner,
) -> e3_events::SignedProofPayload {
    use e3_events::{CircuitName, ProofPayload, ProofType, SignedProofPayload};

    let mut signals = vec![0u8; 96];
    signals[0..32].copy_from_slice(&[0xAA; 32]); // sk_commitment (dummy)
    signals[32..64].copy_from_slice(pk_commitment); // pk_commitment (real)
    signals[64..96].copy_from_slice(&[0xCC; 32]); // e_sm_commitment (dummy)
...
/// Set up the minimal actor stack for C1->C5 tests:
/// event bus + Multithread (with dummy ZkBackend) + ProofRequestActor + HistoryCollector.
async fn setup_c1_c5_actors() -> Result<(
    BusHandle,
    actix::Addr<HistoryCollector<EnclaveEvent>>,
    tempfile::TempDir, // keep alive so paths stay valid
)> {
    let system = EventSystem::new().with_fresh_bus();
    let bus = system.handle()?.enable("c1c5test");

    let history = HistoryCollector::<EnclaveEvent>::new().start();
    bus.subscribe(EventType::All, history.clone().recipient());

    // Dummy ZkBackend — the commitment check runs before any proof generation,
    // so the bb binary and circuit files are never accessed on the mismatch path.
    let temp = tempfile::tempdir()?;
    let temp_path = temp.path();
    let dummy_backend = ZkBackend::new(
        BBPath::Default(temp_path.join("bb")),
        temp_path.join("circuits"),
        temp_path.join("work"),
    );

    // Multithread actor with ZK prover
    let rng = create_shared_rng_from_u64(99);
    let cipher = Arc::new(Cipher::from_password("c1c5-test-key").await?);
    let task_pool = Multithread::create_taskpool(2, 1);
    Multithread::attach_with_zk(&bus, rng, cipher, task_pool, None, &dummy_backend);

    // ProofRequestActor handles PkAggregationProofPending → ComputeRequest dispatch,
    // and ComputeRequestError → SignedProofFailed emission.
    let signer = PrivateKeySigner::random();
    ProofRequestActor::setup(&bus, signer);

    // Enable effects so Multithread subscribes to ComputeRequest
...
/// C1->C5: Tampered commitment is detected as C1CommitmentMismatch.
///
/// Generates 3 real BFV keyshares, builds C1 proofs with correct commitments
/// for parties 0 and 1 but a WRONG commitment for party 2, dispatches
/// PkAggregationProofPending through the actor system, and verifies:
///   - ComputeRequestError with C1CommitmentMismatch appears in history
///   - The mismatched index includes party 2
///   - SignedProofFailed is emitted for the faulting party
///   - E3Failed is NOT emitted (aggregator can retry with honest subset)
#[actix::test]
#[serial_test::serial]
async fn test_c1_c5_tampered_commitment_detected() -> Result<()> {
    let _guard = with_tracing("info");
    let (bus, history, _temp) = setup_c1_c5_actors().await?;

    let e3_id = E3id::new("9001", 1);
    let num_parties = 3;

    // Generate real BFV keyshares and correct commitments
    let (keyshare_bytes, aggregated_pk_bytes, correct_commitments, _params) =
        generate_keyshares_and_commitments(num_parties)?;

    // Build signed C1 proofs: parties 0,1 get correct commitments, party 2 gets a tampered one
    let signers: Vec<_> = (0..num_parties)
        .map(|_| PrivateKeySigner::random())
        .collect();
    let mut c1_proofs = Vec::with_capacity(num_parties);
    for i in 0..num_parties {
        let commitment = if i == 2 {
            [0xFFu8; 32] // tampered commitment
        } else {
            correct_commitments[i]
        };
        c1_proofs.push(build_signed_c1_proof(&e3_id, &commitment, &signers[i]));
    }

    // Dispatch PkAggregationProofPending
    let proof_request = PkAggregationProofRequest {
        keyshare_bytes,
        aggregated_pk_bytes,
        params_preset: BfvPreset::InsecureThreshold512,
        committee_n: num_parties,
        committee_h: num_parties,
        committee_threshold: 1,
        c1_signed_proofs: c1_proofs,
    };

    bus.publish_without_context(PkAggregationProofPending {
        e3_id: e3_id.clone(),
        proof_request,
        public_key: ArcBytes::from_bytes(&[]),
        nodes: OrderedSet::from(vec![
            signers[0].address().to_string(),
            signers[1].address().to_string(),
            signers[2].address().to_string(),
        ]),
    })?;

    // Wait for the async actor chain to complete:
    //   PkAggregationProofPending -> ProofRequestActor -> ComputeRequest
    //   → Multithread -> ComputeRequestError -> ProofRequestActor -> SignedProofFailed
    sleep(Duration::from_secs(5)).await;

    let events = history.send(GetEvents::new()).await?;
    let event_types: Vec<String> = events.iter().map(|e| e.event_type()).collect();

    // 1. ComputeRequestError should be in history (from Multithread)
    assert!(
        event_types.contains(&"ComputeRequestError".to_string()),
        "Expected ComputeRequestError in history, got: {:?}",
        event_types
    );

    // 2. SignedProofFailed should be emitted for the faulting party
    let signed_proof_failed: Vec<_> = events
        .iter()
        .filter(|e| e.event_type() == "SignedProofFailed")
        .collect();
    assert!(
        !signed_proof_failed.is_empty(),
        "Expected SignedProofFailed in history, got: {:?}",
        event_types
    );

    // 3. ProofRequestActor should NOT emit E3Failed for C1CommitmentMismatch —
    //    it suppresses E3Failed so the PublicKeyAggregator can retry with honest parties.
    assert!(
        !event_types.contains(&"E3Failed".to_string()),
        "ProofRequestActor should not emit E3Failed on C1CommitmentMismatch"
    );

    Ok(())
}

/// C1->C5: All commitments correct passes the check (gets past commitment verification).
///
/// With all correct commitments, the commitment check passes and the prover
/// proceeds to actual proof generation — which fails because we use a dummy
/// ZkBackend. We verify the error is NOT a C1CommitmentMismatch.
#[actix::test]
#[serial_test::serial]
async fn test_c1_c5_correct_commitments_pass_check() -> Result<()> {
    let _guard = with_tracing("info");
    let (bus, history, _temp) = setup_c1_c5_actors().await?;

    let e3_id = E3id::new("9002", 1);
    let num_parties = 3;

    let (keyshare_bytes, aggregated_pk_bytes, correct_commitments, _params) =
        generate_keyshares_and_commitments(num_parties)?;

    // All C1 proofs get correct commitments
    let signers: Vec<_> = (0..num_parties)
        .map(|_| PrivateKeySigner::random())
        .collect();
    let c1_proofs: Vec<_> = (0..num_parties)
        .map(|i| build_signed_c1_proof(&e3_id, &correct_commitments[i], &signers[i]))
        .collect();
    ...
    // Wait for processing — the commitment check passes but proof generation
    // fails (dummy backend, no bb binary). We just check it's NOT a mismatch.
    sleep(Duration::from_secs(5)).await;

    let events = history.send(GetEvents::new()).await?;
    let event_types: Vec<String> = events.iter().map(|e| e.event_type()).collect();

    // Should NOT have SignedProofFailed (no commitment mismatch)
    assert!(
        !event_types.contains(&"SignedProofFailed".to_string()),
        "SignedProofFailed should NOT appear when all commitments match, got: {:?}",
        event_types
    );
...
/// C1->C5: All commitments tampered — all indices detected.
///
/// When every party's C1 commitment is wrong, the prover returns
/// C1CommitmentMismatch with all indices. SignedProofFailed is emitted
/// for each party.
#[actix::test]
#[serial_test::serial]
async fn test_c1_c5_all_tampered_all_detected() -> Result<()> {
    let _guard = with_tracing("info");
    let (bus, history, _temp) = setup_c1_c5_actors().await?;

    let e3_id = E3id::new("9003", 1);
    let num_parties = 3;

    let (keyshare_bytes, aggregated_pk_bytes, _correct_commitments, _params) =
        generate_keyshares_and_commitments(num_parties)?;

    // ALL C1 proofs get wrong commitments
    let signers: Vec<_> = (0..num_parties)
        .map(|_| PrivateKeySigner::random())
        .collect();
    let c1_proofs: Vec<_> = (0..num_parties)
        .map(|i| {
            let bad_commitment = [(i + 1) as u8; 32]; // unique wrong value per party
            build_signed_c1_proof(&e3_id, &bad_commitment, &signers[i])
        })
        .collect();
    ...
    // All 3 parties should have SignedProofFailed events
    let failed_count = event_types
        .iter()
        .filter(|t| *t == "SignedProofFailed")
        .count();
    assert_eq!(
        failed_count, num_parties,
        "Expected {} SignedProofFailed events (one per tampered party), got {}. Events: {:?}",
        num_parties, failed_count, event_types
    );

    // NOTE: In the full system, PublicKeyAggregator would receive the
    // ComputeRequestError, determine 0 honest parties remain, and emit
    // E3Failed. This test only covers ProofRequestActor + Multithread
    // (no aggregator), so E3Failed emission from the retry-or-fail path
    // is not exercised here.

    Ok(())
}

/// C1->C5: Fault attribution recovers the correct signer address.
///
/// Verifies that the SignedProofFailed event contains the correct faulting
/// node address (recovered from the ECDSA signature on the tampered C1 proof).
#[actix::test]
#[serial_test::serial]
async fn test_c1_c5_fault_attribution_address_recovery() -> Result<()> {
    let _guard = with_tracing("info");
    let (bus, history, _temp) = setup_c1_c5_actors().await?;

    let e3_id = E3id::new("9004", 1);
    let num_parties = 2;

    let (keyshare_bytes, aggregated_pk_bytes, correct_commitments, _params) =
        generate_keyshares_and_commitments(num_parties)?;

    // Party 0 correct, party 1 tampered
    let faulting_signer = PrivateKeySigner::random();
    let honest_signer = PrivateKeySigner::random();
    let expected_faulting_addr = faulting_signer.address();

    let c1_proofs = vec![
        build_signed_c1_proof(&e3_id, &correct_commitments[0], &honest_signer),
        build_signed_c1_proof(&e3_id, &[0xBB; 32], &faulting_signer), // tampered
    ];
    ...
    bus.publish_without_context(PkAggregationProofPending {
        ...
        public_key: ArcBytes::from_bytes(&[]),
        nodes: OrderedSet::from(vec![
            honest_signer.address().to_string(),
            faulting_signer.address().to_string(),
        ]),
    })?;
    ...
    // Find SignedProofFailed and verify the recovered address
    let failed_events: Vec<_> = events
        .iter()
        .filter_map(|e| match e.clone().into_data() {
            EnclaveEventData::SignedProofFailed(spf) => Some(spf),
            _ => None,
        })
        .collect();
    ...
    assert_eq!(
        failed_events.len(),
        1,
        "Expected exactly 1 SignedProofFailed, got {}",
        failed_events.len()
    );

    assert_eq!(
        failed_events[0].faulting_node, expected_faulting_addr,
        "SignedProofFailed should contain the faulting party's address"
    );
