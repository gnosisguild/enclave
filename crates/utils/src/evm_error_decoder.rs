...
static SELECTOR_MAP: LazyLock<
    HashMap<[u8; 4], (&'static str, &'static [(&'static str, &'static str)])>,
> = LazyLock::new(|| {
    ERROR_SELECTORS
        .iter()
...
pub fn decode_selector(selector: &[u8; 4]) -> Option<DecodedEvmError> {
    SELECTOR_MAP
        .get(selector)
        .map(|(name, params)| DecodedEvmError {
            name: (*name).to_string(),
...
mod tests {
    ...
    fn test_error_selectors_loaded() {
        ...
        assert!(
            !ERROR_SELECTORS.is_empty(),
            "ERROR_SELECTORS should not be empty"
        );
    }
    ...
    fn test_display_impl() {
        ...
        assert_eq!(
            format!("{}", error_with_params),
            "E3DoesNotExist(uint256) [0xabcd1234]"
        );
    }
}
