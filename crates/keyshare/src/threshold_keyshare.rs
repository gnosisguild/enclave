...
use e3_events::{
    ...
    Sequenced, ShareComputationProofRequest, ShareComputationProofSigned,
    ShareEncryptionProofRequest, SignedProofPayload, ThresholdShare,
    ThresholdShareCollectionFailed, ThresholdShareCreated, ThresholdSharePending,
    TypedEvent,
};
use e3_fhe::create_crp;
use e3_fhe_params::{BfvParamSet, BfvPreset, build_pair_for_preset};
use e3_trbfv::{
...
impl ThresholdKeyshare {
        ...
        self.state.try_mutate(&ec, |s| {
            ...
            let updated = match msg.proof_type {
                ...
                ProofType::T1SkShareEncryption => {
                    ...
                    updated.signed_sk_share_encryption_proofs.push(msg.signed_proof);
                    updated
                },
                ProofType::T1ESmShareEncryption => {
                    let mut updated = current;
                    updated.signed_e_sm_share_encryption_proofs.push(msg.signed_proof);
                    updated
                },
                other => {
                    warn!(
    ...
    pub fn handle_shares_generated(&mut self, ec: EventContext<Sequenced>) -> Result<()> {
        ...
        for esi_recipient_witnesses in esi_witnesses.iter() {
            for (recipient_idx, recipient_witnesses) in
                esi_recipient_witnesses.iter().enumerate()
            {
                for (row_idx, witness) in recipient_witnesses.iter().enumerate() {
