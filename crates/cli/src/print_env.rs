...
pub async fn execute(out: Console, config: &AppConfig, chain: &str, as_vite: bool) -> Result<()> {
    if as_vite {
        log!(out, "{}", extract_env_vars_vite(config, chain));
    } else {
        log!(out, "{}", extract_env_vars(config, chain));
    }
    Ok(())
}
