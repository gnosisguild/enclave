...
impl ThresholdPlaintextAggregatorState {
    pub fn init(
        ...
        ThresholdPlaintextAggregatorState::Collecting {
            threshold_m,
            threshold_n,
...
impl ThresholdPlaintextAggregator {
    ...
    pub fn add_share(&mut self, share: Vec<u8>) -> Result<()> {
        self.state.try_mutate(|mut state| {
            let ThresholdPlaintextAggregatorState::Collecting {
                threshold_m,
                shares,
                ciphertext_output,
                ..
            ...
            if shares.len() == *threshold_m {
                return Ok(ThresholdPlaintextAggregatorState::Computing {
                    shares: shares.clone(),
    ...
    pub fn set_decryption(&mut self, decrypted: Vec<u8>) -> Result<()> {
        self.state.try_mutate(|mut state| {
            let ThresholdPlaintextAggregatorState::Computing { shares, .. } = &mut state else {
                return Ok(state.clone());
            };
            let shares = shares.to_owned();

            Ok(ThresholdPlaintextAggregatorState::Complete { decrypted, shares })
        })
    }
}
...
impl Handler<DecryptionshareCreated> for ThresholdPlaintextAggregator {
    ...
    fn handle(&mut self, event: DecryptionshareCreated, _: &mut Self::Context) -> Self::Result {
        let Some(ThresholdPlaintextAggregatorState::Collecting {
            threshold_n, seed, ..
        }) = self.state.get()
        else {
        ...
        Box::pin(
            self.sortition
                ...
                .map(move |res, act, ctx| {
                    ...
                    if let Some(ThresholdPlaintextAggregatorState::Computing {
                        shares,
                        ciphertext_output,
                    }) = &act.state.get()
                    {
