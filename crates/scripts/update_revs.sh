...
find . -name "Cargo.toml" -not -path "$EXCLUDE_PATH" -exec grep -l "git = \"$GITHUB_REPO_URL" {} \; | while read -r file; do
    echo "File: $file"
...
find . -name "Cargo.toml" -not -path "$EXCLUDE_PATH" -exec sed -i "s|rev = \"[^\"]*\"|rev = \"$CURRENT_HASH\"|g" {} \;
echo "Done!"
