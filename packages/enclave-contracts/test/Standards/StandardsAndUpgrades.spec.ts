...
//     Enclave, registries or the refund manager. Add an IERC1271 check if
...
import {
  ...
  IEnclave__factory as IEnclaveFactory,
...
describe("Standards & upgradeability hygiene", function () {
  describe("ERC-165 supportsInterface", function () {
    it("Enclave: supports IEnclave + IERC165, rejects 0xffffffff", async function () {
      const { enclave } = await deployAll();
      const iEnclaveId = interfaceIdOf(IEnclaveFactory.createInterface());
      expect(await enclave.supportsInterface(iEnclaveId)).to.equal(true);
      expect(await enclave.supportsInterface(IERC165_ID)).to.equal(true);
      expect(await enclave.supportsInterface(INVALID_ID)).to.equal(false);
    });
  ...
  describe("EIP-6372 clock advertisement (ERC20Votes)", function () {
    it("EnclaveTicketToken: CLOCK_MODE() and clock() report timestamp mode", async function () {
    ...
    it("EnclaveToken: CLOCK_MODE() and clock() report timestamp mode", async function () {
  ...
  describe("storage gaps on upgradeable contracts", function () {
    ...
    it("all upgradeable contracts deploy without storage collision", async function () {
      ...
      expect(await all.enclave.getAddress()).to.properAddress;
