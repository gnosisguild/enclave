...
describe("E3 Integration - Refund/Timeout Mechanism", function () {
  ...
  describe("Work Value Verification", function () {
    it("verifies work allocation percentages align with BlockScience spec", async function () {
      const { e3RefundManager } = await loadFixture(setup);

      const allocation = await e3RefundManager.getWorkAllocation();

      expect(allocation.committeeFormationBps).to.equal(1000);
      expect(allocation.dkgBps).to.equal(3000);
      expect(allocation.decryptionBps).to.equal(5500);
      expect(allocation.protocolBps).to.equal(500);

      const total =
        Number(allocation.committeeFormationBps) +
        Number(allocation.dkgBps) +
        Number(allocation.decryptionBps) +
        Number(allocation.protocolBps);

      expect(total).to.equal(10000);
    });

    it("calculates correct work value at each stage", async function () {
      const { e3RefundManager } = await loadFixture(setup);

      // Requested
      let [workCompleted, workRemaining] =
  ...
  describe("Events Verification", function () {
    it("emits E3StageChanged on stage transitions", async function () {
      const { e3Lifecycle, makeRequest } = await loadFixture(setup);

      // Request triggers initializeE3 which emits None -> Requested
      const { e3Id } = await makeRequest();

      // Verify state changed to Requested
      const stage = await e3Lifecycle.getE3Stage(e3Id);
      expect(stage).to.equal(1); // E3Stage.Requested
    });

    it("emits E3Failed on failure", async function () {
      const { e3Lifecycle, makeRequest } = await loadFixture(setup);
    ...
    it("emits E3FailureProcessed on processing", async function () {
      const { enclave, e3Lifecycle, makeRequest } = await loadFixture(setup);
    ...
    it("emits RefundClaimed on claim", async function () {
      const { enclave, e3Lifecycle, e3RefundManager, makeRequest, requester } =
        await loadFixture(setup);
  ...
  describe("Stage Transition Verification", function () {
    it("tracks deadlines correctly through all stages", async function () {
      ...
      let deadlines = await e3Lifecycle.getDeadlines(0);
      expect(deadlines.committeeDeadline).to.be.gte(
        requestTime + defaultTimeoutConfig.committeeFormationWindow,
      );

      // Committee Formed
      const activationDeadline = (await time.latest()) + SEVEN_DAYS;
      await e3Lifecycle.setEnclave(await owner.getAddress());
      const committeeFormedTime = await time.latest();
      await e3Lifecycle.connect(owner).onCommitteeFinalized(0);
      await e3Lifecycle.connect(owner).onKeyPublished(0, activationDeadline);

      deadlines = await e3Lifecycle.getDeadlines(0);
      expect(deadlines.dkgDeadline).to.be.gte(
        committeeFormedTime + defaultTimeoutConfig.dkgWindow,
      );
      ...
      const ciphertextTime = await time.latest();
      await e3Lifecycle.connect(owner).onCiphertextPublished(0);
      await e3Lifecycle.setEnclave(await enclave.getAddress());

      deadlines = await e3Lifecycle.getDeadlines(0);
      expect(deadlines.decryptionDeadline).to.be.gte(
        ciphertextTime + defaultTimeoutConfig.decryptionWindow,
      );
    });

    it("prevents invalid stage transitions", async function () {
      const { e3Lifecycle, makeRequest, owner, enclave } =
        await loadFixture(setup);

      await makeRequest();

      // Try to skip directly to Activated (should fail)
      await e3Lifecycle.setEnclave(await owner.getAddress());
      await expect(
        e3Lifecycle
          .connect(owner)
      ...
      // Try to skip to CiphertextPublished (should fail)
      await expect(
        e3Lifecycle.connect(owner).onCiphertextPublished(0),
      ).to.be.revertedWithCustomError(e3Lifecycle, "InvalidStage");
  ...
  describe("Failure Reason Attribution", function () {
    it("correctly attributes failure to CommitteeFormationTimeout", async function () {
      const { e3Lifecycle, makeRequest } = await loadFixture(setup);
    ...
    it("correctly attributes failure to DKGTimeout", async function () {
      const { e3Lifecycle, makeRequest, owner, enclave } =
        await loadFixture(setup);

      await makeRequest();

      // For DKG timeout, committee is finalized but key is never published
      await e3Lifecycle.setEnclave(await owner.getAddress());
      await e3Lifecycle.connect(owner).onCommitteeFinalized(0);
      await e3Lifecycle.setEnclave(await enclave.getAddress());
    ...
    it("correctly attributes failure to ActivationWindowExpired", async function () {
      const { e3Lifecycle, makeRequest, owner, enclave } =
        await loadFixture(setup);

      await makeRequest();

      // Set a very short activation deadline
      const activationDeadline = (await time.latest()) + 10; // Only 10 seconds
      await e3Lifecycle.setEnclave(await owner.getAddress());
      await e3Lifecycle.connect(owner).onCommitteeFinalized(0);
      await e3Lifecycle.connect(owner).onKeyPublished(0, activationDeadline);
      await e3Lifecycle.setEnclave(await enclave.getAddress());

      await time.increase(11); // Past activation deadline but before DKG deadline
      await e3Lifecycle.markE3Failed(0);
    ...
    it("correctly attributes failure to ComputeTimeout", async function () {
      const { e3Lifecycle, makeRequest, owner, enclave } =
        await loadFixture(setup);
    ...
    it("correctly attributes failure to DecryptionTimeout", async function () {
      const { e3Lifecycle, makeRequest, owner, enclave } =
        await loadFixture(setup);
  ...
  describe("Refund Distribution Verification", function () {
    it("allocates correct refund for committee formation timeout (0% work)", async function () {
      ...
      expect(distribution.requesterAmount).to.be.gt(
        distribution.protocolAmount,
      );
    });

    it("allocates correct refund for DKG timeout (10% work)", async function () {
      const { enclave, e3Lifecycle, e3RefundManager, makeRequest, owner } =
        await loadFixture(setup);
    ...
    it("allocates correct refund for decryption timeout (40% work)", async function () {
      const { enclave, e3Lifecycle, e3RefundManager, makeRequest, owner } =
        await loadFixture(setup);
  ...
  describe("Edge Cases and Error Handling", function () {
    it("reverts when marking non-existent E3 as failed", async function () {
      const { e3Lifecycle } = await loadFixture(setup);

      await expect(e3Lifecycle.markE3Failed(999)).to.be.revertedWithCustomError(
        e3Lifecycle,
        "InvalidStage",
      );
    });

    it("reverts when claiming refund for non-failed E3", async function () {
    ...
    it("reverts when non-requester tries to claim requester refund", async function () {
      const { enclave, e3Lifecycle, e3RefundManager, makeRequest, owner } =
        await loadFixture(setup);
      ...
      await expect(
        e3RefundManager.connect(owner).claimRequesterRefund(0),
      ).to.be.revertedWithCustomError(e3RefundManager, "NotRequester");
    });

    it("reverts when failure condition not met", async function () {
      const { e3Lifecycle, makeRequest } = await loadFixture(setup);

      await makeRequest();

      // Try to mark as failed immediately (deadline not passed)
    ...
    it("reverts when trying to process already processed failure", async function () {
      const { enclave, e3Lifecycle, makeRequest } = await loadFixture(setup);
    ...
    it("handles zero honest nodes gracefully", async function () {
      const { enclave, e3Lifecycle, e3RefundManager, makeRequest } =
        await loadFixture(setup);
      ...
      // With no honest nodes, honestNodeAmount should still be calculated
      // but claiming would fail as there are no honest nodes
      expect(distribution.calculated).to.be.true;
    });
  });
