...
describe("Committee Expulsion & Fault Tolerance", function () {
  ...
  describe("committee expulsion via proposeSlash", function () {
    ...
    it("should fail E3 when active members drop below threshold", async function () {
      ...
      // Use operator1 and operator3 as voters (operator1's vote still valid per isCommitteeMember)
    ...
    it("should handle idempotent expulsion (re-slashing same node)", async function () {
      ...
      // Slash operator1 again with different evidence (different dataHash)
      const proof2 = await signAndEncodeAttestation(
        [operator2, operator3],
        0,
        await operator1.getAddress(),
  ...
  describe("E3 fails below threshold", function () {
    it("should fail E3 exactly at the threshold breach", async function () {
      ...
      // Both committee members vote (including the accused — contract allows it)
      const proof = await signAndEncodeAttestation(
        [operator1, operator2],
        0,
        await operator1.getAddress(),
    ...
    it("should not fail E3 twice on multiple sub-threshold expulsions", async function () {
      ...
      // operator1 is expelled but still isCommitteeMember, so can vote
      const proof2 = await signAndEncodeAttestation(
        [operator1, operator3],
        0,
        await operator2.getAddress(),
      ...
      const proof3 = await signAndEncodeAttestation(
        [operator1, operator2],
        0,
