...
interface ICiphernodeRegistry {
    ...
    /// @dev Any caller may submit the first valid proof. Duplicate submissions must revert.
    /// @param e3Id ID of the E3 for which to select the committee.
    ...
    /// @param proof C5 proof; ABI-encoded (bytes rawProof, bytes32[] publicInputs). Last input is aggregate commitment.
    ...
    /// @notice Read the published public key hash directly without reverting when it is unset.
    /// @param e3Id ID of the E3.
    /// @return publicKeyHash The currently stored committee public key hash, or zero when unpublished.
    function publicKeyHashes(
        uint256 e3Id
    ) external view returns (bytes32 publicKeyHash);
    ...
    /// @return committeeScores The score of nodes in the committee for a given E3.
    function getCommitteeNodes(
        ...
        view
        returns (
            address[] memory committeeNodes,
            uint256[] memory committeeScores
        );
    ...
    /// @notice Get the current lifecycle stage for an E3 committee.
    /// @param e3Id ID of the E3 computation
    /// @return stage The current committee stage
    function getCommitteeStage(
        uint256 e3Id
    ) external view returns (CommitteeStage stage);
