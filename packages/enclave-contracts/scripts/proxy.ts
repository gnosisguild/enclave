 ...
 * This is where TransparentUpgradeableProxy stores the implementation address
...
/**
 * Gets the implementation address from a TransparentUpgradeableProxy
 * @param provider The ethers provider
 * @param proxyAddress The address of the proxy contract
...
export async function getProxyImplementation(
  ...
  // Extract the address from the storage slot (last 20 bytes)
  const addressHex = "0x" + implementationSlotValue.slice(-40);
