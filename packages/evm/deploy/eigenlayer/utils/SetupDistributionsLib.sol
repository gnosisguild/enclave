...
library SetupDistributionsLib {
    ...
    function createAVSRewardsSubmissions(
        ...
        IRewardsCoordinatorTypes.RewardsSubmission[]
            memory rewardsSubmissions = new IRewardsCoordinatorTypes.RewardsSubmission[](
                numPayments
            );
        for (uint256 i = 0; i < numPayments; i++) {
            ...
            IRewardsCoordinatorTypes.RewardsSubmission
                memory rewardsSubmission = IRewardsCoordinatorTypes
                    .RewardsSubmission({
        ...
        ECDSAServiceManagerBase(helloWorldServiceManager)
    ...
    function createOperatorDirectedAVSRewardsSubmissions(
        ...
        IRewardsCoordinatorTypes.OperatorDirectedRewardsSubmission[]
            memory rewardsSubmissions = new IRewardsCoordinatorTypes.OperatorDirectedRewardsSubmission[](
                numPayments
            );
        for (uint256 i = 0; i < numPayments; i++) {
            IRewardsCoordinatorTypes.StrategyAndMultiplier[]
    ...
    function processClaim(
        ...
        bytes memory proof = generateMerkleProof(
            paymentLeaves.leaves,
            indexToProve
        );
        //we only have one token leaf
        bytes memory tokenProof = generateMerkleProof(
            paymentLeaves.tokenLeaves,
            0
        );
        ...
        IRewardsCoordinatorTypes.TokenTreeMerkleLeaf[]
            memory tokenLeaves = new IRewardsCoordinatorTypes.TokenTreeMerkleLeaf[](
                NUM_TOKEN_EARNINGS
            );
        ...
        IRewardsCoordinatorTypes.RewardsMerkleClaim
            memory claim = IRewardsCoordinatorTypes.RewardsMerkleClaim({
    ...
    function createPaymentRoot(
        ...
        require(
            earnerLeaves.length == NUM_PAYMENTS,
            "Number of earners must match number of payments"
        );
        ...
        returns (IRewardsCoordinatorTypes.EarnerTreeMerkleLeaf[] memory)
    {
        IRewardsCoordinatorTypes.EarnerTreeMerkleLeaf[]
            memory leaves = new IRewardsCoordinatorTypes.EarnerTreeMerkleLeaf[](
                earners.length
            );
    ...
    function createTokenLeaves(
        ...
        for (uint256 i = 0; i < NUM_TOKEN_EARNINGS; i++) {
            IRewardsCoordinatorTypes.TokenTreeMerkleLeaf
                memory leaf = defaultTokenLeaf(TOKEN_EARNINGS, strategy);
    ...
    {
        IRewardsCoordinatorTypes.TokenTreeMerkleLeaf
            memory leaf = IRewardsCoordinatorTypes.TokenTreeMerkleLeaf({
                token: IStrategy(strategy).underlyingToken(),
    ...
    function generateMerkleProof(
        ...
        for (uint256 i = 0; i < depth; i++) {
            ...
            for (uint256 j = 0; j < levelSize; j++) {
                if (2 * j + 1 < n) {
                    leaves[j] = keccak256(
                        abi.encodePacked(leaves[2 * j], leaves[2 * j + 1])
                    );
                } else {
                    leaves[j] = leaves[2 * j];
                }
