...
pub trait WithPrefix: Sized {
    fn with_namespace(self, prefix: &str) -> Self;
}
...
impl WithPrefix for Insert {
    fn with_namespace(self, prefix: &str) -> Self {
        Insert(self.0.with_namespace(prefix), self.1)
    }
}
...
impl WithPrefix for Get {
    fn with_namespace(self, prefix: &str) -> Self {
        Get(self.0.with_namespace(prefix))
    }
}
...
impl WithPrefix for DataStore {
    fn with_namespace(self, prefix: &str) -> Self {
        Self {
            get: self.get,
            insert: self.insert,
            prefix: self.prefix.map_or(Some(prefix.to_string()), |p| {
                Some(vec![prefix.to_string(), p].join("/"))
            }),
        }
    }
}
